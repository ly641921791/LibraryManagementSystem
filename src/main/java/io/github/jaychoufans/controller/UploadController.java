...
public class UploadController {
	...
	public Map<String, Object> uploadFile(MultipartFile file) throws Exception {
		...
		if (!file.isEmpty()) {
			...
			map.put("code", 0);
			map.put("msg", "上传成功");
			System.out.println("http://localhost:8080/uploads/" + new SimpleDateFormat("yyyy/MM/dd").format(date) + "/"
					+ dir1 + "/" + dir2 + "/" + newFileName);
		}
		return map;
	}
	...
	public String ckeditorUpload(@RequestParam("upload") MultipartFile file, String CKEditorFuncNum)
		...
		sb.append("window.parent.CKEDITOR.tools.callFunction(" + CKEditorFuncNum + ",'" + "/uploads/"
				+ new SimpleDateFormat("yyyy/MM/dd").format(date) + "/" + dir1 + "/" + dir2 + "/" + newFileName
				+ "','')");
		sb.append("</script>");
		return sb.toString();
	}
